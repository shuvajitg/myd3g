    ...
    useEffect(() => {
        ...
        const indexedData = stocks.map((stock: keyof typeof data) => ({
            ...
            values: data[stock as keyof typeof data].map((d: StockDataPoint) => ({
                x: new Date(d.x),
                y: d.y / data[stock][0].y, // Normalize values
            })),
        }));
        ...
        const xScale = d3
            .scaleUtc()
            .domain((d3.extent(indexedData[0].values, (d: any) => d.x) as [Date | undefined, Date | undefined]).map(d => d ?? new Date()) as [Date, Date])
        ...
        const yScale = d3
            .scaleLinear()
            .domain([
                d3.min(indexedData.flatMap((stock) => stock.values.map((d: any) => d.y)))! - 0.2,
                d3.max(indexedData.flatMap((stock) => stock.values.map((d: any) => d.y)))! + 0.2,
            ])
        ...
        indexedData.forEach((stock: any) => {
            ...
            // Add text labels
            svg.append("text")
                .attr("x", xScale(stock.values[stock.values.length - 1].x) + 5)
                .attr("y", yScale(stock.values[stock.values.length - 1].y))
